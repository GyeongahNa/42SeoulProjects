...
void getbasicinfo(int nearest, int cnt)
{
	...
	raydistance = distancepoints(g_player_x, g_player_y, g_sprtinfo[cnt]->node_x, g_sprtinfo[cnt]->node_y);
...
void getinfo(int nearest, int cnt)
{
	...
	while (columnid < g_num_rays)
	{
		if ((g_rayinfo[columnid]->map_x == x) && (g_rayinfo[columnid]->map_y == y))
		{
	...
	while ((columnid < g_num_rays) && (g_rayinfo[columnid]->map_x == x) && (g_rayinfo[columnid]->map_y == y))
		columnid++;
	if (columnid == 0)
		columnid = columnid + 1;
