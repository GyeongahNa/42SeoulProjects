...
int getcenter(int num)
{
	...
	if (g_sprtinfo[num]->start > 0 && g_sprtinfo[num]->end < g_num_rays - 1)
		center = g_sprtinfo[num]->start + 0.5 * g_sprtinfo[num]->wallheight;
	else if (g_sprtinfo[num]->start == 0 && g_sprtinfo[num]->end == g_num_rays - 1)
		center = g_window_width / 2;
	else if ((g_sprtinfo[num]->end - g_sprtinfo[num]->start) / g_sprtinfo[num]->wallheight > 0.98)
		center = g_sprtinfo[num]->start + 0.5 * g_sprtinfo[num]->wallheight;
	else if (g_sprtinfo[num]->start == 0 && g_sprtinfo[num]->end < g_num_rays - 1)
		center = g_sprtinfo[num]->end - 0.5 * g_sprtinfo[num]->wallheight;
	else if (g_sprtinfo[num]->start > 0 && g_sprtinfo[num]->end == g_num_rays - 1)
		center = g_sprtinfo[num]->start + 0.5 * g_sprtinfo[num]->wallheight;
	else
		center = 0;
...
void renderpart(int columnid, double dist, int color)
{
	...
	while (k < start + wallheight)
	{
		if (k >= 0 && k < g_window_height)
		{
			...
			color = g_texture[gettexture(columnid)][texx + TEXWIDTH * texy];
			if (dist > 1)
...
void renderheight(int i, int rowstart, int num, int texx)
{
	...
	while (j < rowend)
	{
		texy = (j - rowstart) / g_sprtinfo[num]->wallheight * TEXHEIGHT;
		if (i >= 0 && i <= g_window_width - 1 && j >= 0 && j <= g_window_height - 1)
		{
			dist = distancepoints(g_sprtinfo[num]->node_x, g_sprtinfo[num]->node_y, g_player_x, g_player_y) * 0.003;
