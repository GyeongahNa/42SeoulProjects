...
/*   Updated: 2020/09/06 00:18:39 by gna              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int caltexx(int columnid)
{
	...
	texx = (g_rayinfo[columnid]->washorzhit == 1)
			   ? (int)((fmod(g_rayinfo[columnid]->wallhitx, g_tile_size) / g_tile_size * TEXWIDTH))
			   : (int)((fmod(g_rayinfo[columnid]->wallhity, g_tile_size) / g_tile_size * TEXWIDTH));
	return (texx);
}
	...
	while (col < g_num_rays)
	{
		...
		while (k < start + wallheight)
		{
			if (k >= 0 && k < g_window_height)
			{
				...
				g_image_data[col + g_window_width * k] = (d <= 1
															  ? g_texture[gettexture(col)][texx + TEXWIDTH * texy]
															  : shadow(g_texture[gettexture(col)][texx + TEXWIDTH * texy], d));
			}
			k++;
		}
		col++;
