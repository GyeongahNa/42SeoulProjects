...
void renderrays(int i, int j, double k)
{
	...
	while (columnid < g_num_rays)
	{
		...
		while (k < MINIMAP_SCALE_FACTOR *
					   sqrt((g_player_x - wallhitx) * (g_player_x - wallhitx) + (g_player_y - wallhity) * (g_player_y - wallhity)))
		{
			i = g_player_x * MINIMAP_SCALE_FACTOR + cos(g_rayinfo[columnid]->rayangle) * k;
			j = g_player_y * MINIMAP_SCALE_FACTOR + sin(g_rayinfo[columnid]->rayangle) * k;
			g_image_data[i + g_window_width * j] = 255 * 256 * 256 + 102 * 256 + 51;
			k = k + 0.05;
		}
		columnid++;
	}
