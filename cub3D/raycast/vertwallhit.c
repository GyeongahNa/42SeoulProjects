...
double calxintercept(int columnid)
{
	...
	xintercept += ((g_rayinfo[columnid]->dir.israyfacingright == 1) ? (double)g_tile_size : 0);
	xintercept = ((g_rayinfo[columnid]->dir.israyfacingleft) ? xintercept - 1.0 : xintercept);
	return (xintercept);
}
...
double calystep(int columnid)
{
	...
	ystep *= (((g_rayinfo[columnid]->dir.israyfacingup) && (ystep > 0)) ? -1.0 : 1.0);
	ystep *= (((g_rayinfo[columnid]->dir.israyfacingdown) && (ystep < 0)) ? -1.0 : 1.0);
	return (ystep);
}

void savevertinfo(int columnid, int xintercept, int yintercept, int texnum)
{
	...
	g_rayinfo[columnid]->vertcoord.map_x = (int)(floor(xintercept / g_tile_size));
	g_rayinfo[columnid]->vertcoord.map_y = (int)(floor(yintercept / g_tile_size));
}
...
void vertwallhit(int col, int **map, int start, int end)
{
	...
	xst = g_tile_size * ((g_rayinfo[col]->dir.israyfacingleft) ? -1.0 : 1.0);
	yst = calystep(col);
	while ((xinter > 0) && (xinter < g_window_width && yinter > 0) && (yinter < g_window_height))
	{
		texnum = map[(int)floor((yinter / g_tile_size))][(int)floor((xinter / g_tile_size))];
