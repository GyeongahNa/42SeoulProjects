...
double calyintercept(int columnid)
{
	...
	yintercept += ((g_rayinfo[columnid]->dir.israyfacingdown == 1) ? (double)g_tile_size : 0);
	yintercept = ((g_rayinfo[columnid]->dir.israyfacingup == 1) ? yintercept - 1.0 : yintercept);
	return (yintercept);
}
...
double calxstep(int columnid)
{
	...
	xstep *= (((g_rayinfo[columnid]->dir.israyfacingleft) && xstep > 0) ? -1 : 1);
	xstep *= (((g_rayinfo[columnid]->dir.israyfacingright) && xstep < 0) ? -1 : 1);
	return (xstep);
}
...
void horzwallhit(int columnid, int **map, int start, int end)
{
	...
	xinter = g_player_x + (yinter - g_player_y) / (double)tan(g_rayinfo[columnid]->rayangle);
